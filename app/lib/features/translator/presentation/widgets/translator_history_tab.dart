...
class TranslatorHistoryTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final AsyncValue<List<TranslationDto>> items =
        ref.watch(historyProvider(favoritesOnly));
    return items.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (Object e, _) => Center(child: Text('$e')),
      data: (List<TranslationDto> list) {
        if (list.isEmpty) {
          return Center(child: Text(favoritesOnly
              ...
              Padding(
                ...
                child: Align(
                  ...
                  child: TextButton.icon(
                    onPressed: () async {
                      await ref.read(translatorRepositoryProvider).clearHistory();
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (BuildContext c, int i) {
                  ...
                  return Card(
                    child: ListTile(
                      title: Text(t.sourceText, maxLines: 2, overflow: TextOverflow.ellipsis),
                      subtitle: Text(t.translatedText,
                      ...
                      trailing: Wrap(
                        ...
                        children: <Widget>[
                          IconButton(
                            ...
                            onPressed: () async {
                              await ref.read(translatorRepositoryProvider).toggleFavorite(t.id);
                              ref.invalidate(historyProvider(false));
                          ...
                          IconButton(
                            icon: const Icon(Icons.delete_outline),
                            onPressed: () async {
                              await ref.read(translatorRepositoryProvider).deleteItem(t.id);
                              ref.invalidate(historyProvider(false));
                              ref.invalidate(historyProvider(true));
                            },
                          ),
